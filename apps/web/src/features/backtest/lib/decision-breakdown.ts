 ...
 * public descriptor), resolved at runtime from `GET /strategies`. The web no
 * longer hardcodes a per-strategy copy (core invariant #1): it names a blocker's
 * lever off whatever the active strategy provides.
 */
 ...
 * map (the strategy's own declaration), reading the run's config so the panel
 * names the exact field and its current value. Picks the first armed path; a
