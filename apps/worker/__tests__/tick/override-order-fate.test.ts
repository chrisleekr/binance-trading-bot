...
const OVERRIDE = 'ov-1';
const BREAKER = 'daily-loss breaker';
...
describe('resolveOverrideOrderFate over a replacement', () => {
  it('attributes a replacement carrying the override id', () => {
    expect(
      resolveOverrideOrderFate(OVERRIDE, [applied(replacement(OVERRIDE), true)], [], BREAKER),
    ).toEqual({ kind: 'placed' });
  });
  ...
  it('reports the replacement’s own failure rather than "the strategy did not act"', () => {
    const fate = resolveOverrideOrderFate(
      OVERRIDE,
      [applied(replacement(OVERRIDE), false)],
      [],
      BREAKER,
    );
  ...
  it('sees a replacement the breaker dropped as suppressed, not absent', () => {
    // The reason rides through, because with three breakers the operator otherwise learns that something suppressed the order and not which one.
    expect(resolveOverrideOrderFate(OVERRIDE, [], [replacement(OVERRIDE)], BREAKER)).toEqual({
      kind: 'suppressed',
      reason: BREAKER,
    });
  });
  ...
  it('still ignores a replacement carrying no override id', () => {
    expect(resolveOverrideOrderFate(OVERRIDE, [applied(replacement(), true)], [], BREAKER)).toEqual(
      {
        kind: 'none',
      },
    );
