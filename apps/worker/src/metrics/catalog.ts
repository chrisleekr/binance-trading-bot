...
export const CATALOG: Readonly<Record<MetricName, MetricSpec>> = {
  ...
  // The sweep walks profiles serially, so a run that stalled on the third of ten profiles reported the same tail log as a run where the other seven had nothing to repair. Outcome is a closed set of swept | failed | timeout | unswept: `timeout` separates a profile whose query hit the per-profile budget (the starvation shape) from one whose query simply errored, because the two need different responses. No profileId label: the count is a fleet-level health signal and the per-profile identity already rides the warn log.
  archive_recovery_sweep_profiles_total: {
    kind: 'counter',
