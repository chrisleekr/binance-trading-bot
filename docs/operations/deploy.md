...
flowchart TD
    ...
    RoleRun["api / worker / study:<br/>run migrate runner"]:::act
    Lock["Postgres advisory lock<br/>serializes callers"]:::core
    MaintenanceSkip["Explicit offline maintenance:<br/>skip"]:::wait
    AppStart["App starts"]:::act

    Start --> Entry --> SkipGate
    SkipGate -- "no" --> RoleRun --> Lock --> AppStart
    SkipGate -- "yes" --> MaintenanceSkip --> AppStart
...
| `ReconcilePositionRemoved` | critical | The reconciler deleted a position the strategy believed it held (`reconcile_position_removed_total{heldBefore="nonzero"} > 0`, read as a raw level rather than an `increase()`, because a prom-client child is born holding its first write so the rise Prometheus needs to see never happens), by either of its two destructive routes: `flatten-sub-notional-dust` when the wallet backs the position with residue too small to sell, or `prune-phantom-ledger` when no wallet balance backs a cost-basis row at all. Both empty the strategy body **and** delete the `avg_entry_prices` row, and nothing restores that cost basis automatically. The `heldBefore="nonzero"` gate is the whole rule: the same counter also records `heldBefore="zero"`, which is the reconciler converging a row that was already empty after an external sale — routine on ordinary boots, and paging on it is how this alert would get muted. `nonzero` is the case where the bot overruled its own books on evidence assembled from a once-per-profile wallet snapshot and a cached price, either of which can be wrong. No `for:`: the delete has already happened. Reading the raw level means the alert **latches** — the counter never decreases, so it fires until you silence it or the worker restarts, which is honest for an event nothing undoes. A restart clears it silently, so treat a restart during an open instance as an unacknowledged incident. Read the `reconcileHeldQuantity` warn for the symbol, which carries the wallet, the reference price, and the notional floor the decision used. |
| `ReconcileValueBoundDisarmed` | warning | Three or more reconcile passes in two hours reached a dust value bound and could not evaluate it (`reconcile_value_bound_disarmed_total`), so the passes reporting `no-op` for that symbol are not evidence its position is sound. Counted in passes rather than wall-clock because the backstop cron re-runs every 15 minutes: a cold boot legitimately disarms every symbol once, and firing on that would page on every deploy. `reason` routes the fix — `no-reference-price` is a symbol neither the miniTicker cache nor the batched REST fallback could price, `no-min-notional` is a stale exchange-info refresh, `no-wallet-total` is a balance leg Binance returned that would not parse as a finite decimal, which makes the reconciler skip the symbol before any bound is reached. A third emitter shares this series and is not periodic: the operator-triggered `apply-avg-entry-price` job. If the count rose with no boot and no backstop pass to explain it, grep the worker log for `pipeline_apply_avg_entry_price_value_bound_disarmed` on the same symbol — that is an operator saving an entry price against a cold ticker cache, not a sweep, and the pass-counting rationale above does not bound it. |
| `PipelineApplySeedGateStoodDown` | warning | An operator entry-price apply could not evaluate the position seed gate and fell back to the recorded quantity (`pipeline_apply_seed_gate_stood_down_total`). The rule retains `profileId`, `symbol`, and `reason`, holds for 15 minutes, and resolves two hours after the last stand-down. `reason` routes the response: `no-client` means the resolver threw or returned no client, so verify that the account and API-key rows exist, check database health, and inspect `pipeline_apply_avg_entry_price_gate_unavailable` in the worker warning log; `no-symbol-info` means check Redis and the `exchange-info-refresh` job; `bad-symbol-info` means refresh and inspect the cached symbol info; `getaccount-failed` means check API-key validity, the IP allowlist, Binance reachability, and the same worker log. A restart can produce `1` before and after, which no counter-value comparison can distinguish, so the restart arm joins each positive child to `process_start_time_seconds{job="worker"}` on `job` and `instance` before aggregation. This requires the documented worker scrape job name; the process-start branch expires after two hours, while the offset-subtraction arm carries later events. |
