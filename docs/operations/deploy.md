...
`deploy/observability/alerts.yml` ships fifteen Prometheus alerting rules, in two groups: `binance-trading-bot.workers` for the trading and audit paths, `binance-trading-bot.infra` for the process pressures underneath them. Load the file into your Prometheus stack; the Alertmanager, PagerDuty and Slack receiver wiring is yours to own.
