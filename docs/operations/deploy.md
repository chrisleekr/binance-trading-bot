...
| `ReconcilePositionRemoved` | critical | The reconciler deleted a position the strategy believed it held (`reconcile_position_removed_total{heldBefore="nonzero"}` increased in 15 minutes), by either of its two destructive routes: `flatten-sub-notional-dust` when the wallet backs the position with residue too small to sell, or `prune-phantom-ledger` when no wallet balance backs a cost-basis row at all. Both empty the strategy body **and** delete the `avg_entry_prices` row, and nothing restores that cost basis automatically. The `heldBefore="nonzero"` gate is the whole rule: the same counter also records `heldBefore="zero"`, which is the reconciler converging a row that was already empty after an external sale — routine on ordinary boots, and paging on it is how this alert would get muted. `nonzero` is the case where the bot overruled its own books on evidence assembled from a once-per-profile wallet snapshot and a cached price, either of which can be wrong. No `for:`: the delete has already happened. Read the `reconcileHeldQuantity` warn for the symbol, which carries the wallet, the reference price, and the notional floor the decision used. |
