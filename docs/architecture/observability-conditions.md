...
| 2   | `profile-active`    | `profiles.enabled`, plus the three entry-halt flags in Redis       |
...
The two exit rungs split one question in half. `exit-blockers` reports what every held coin is waiting on, with the rung's own threshold read straight off the producer's `detail`, but only raises a finding for a reason that is a fault (`sell-disabled`, `exit-unsellable`, `exit-config-invalid`, `stop-infeasible-dust`, `native-trail-unavailable`) — a coin waiting for its sell trigger is a held position doing its job, and raising a finding for it would hand every healthy profile a headline about selling. A second, narrower route exists beside that set: a **persistence tier**, which promotes a reason to a finding only once one coin has held it for a stated duration. `protective-stop-unplaced` is its only member today, at **15 minutes**. That reason is correct for a moment after a fresh entry, when the stop has not been placed yet, and alarming when it lasts, because it then means every attempt to arm the stop is being refused and the position has nothing on the exchange that would sell it. Only the duration separates the two readings, and `condition_states.since` is what supplies it: `recordCondition` keeps `since` while the code is unchanged and restarts it when the code changes, so the span is an exact clock on the state rather than a count of ticks. The tier filters group MEMBERS, not whole groups, so a profile that is mid-entry on one coin and stuck on another reports only the stuck one in the item's count and symbols. Fault codes never meet that comparison at all: they are actionable the moment they are recorded, and running them through the same filter at a threshold of zero would drop an existing finding whenever a row was dated ahead of the reader's clock. `exit-protection` then asks the separate question of whether each held coin is actually protected on the downside. That is two failures, not one: no exit configured below the entry price at all, read from `detail.hasDownsideExit` rather than re-derived so the report and the bot cannot disagree about it; and an exit that is configured but that the exchange will not accept, read from open `protective-stop-blocked` rows. The second raises a finding on its own, because a stop that was never placed leaves nothing on the exit side to report it. A failed replacement is reconciled with cause `replace-order-failed`.

Each rung resolves to `ok`, `finding`, `skipped`, or `unknown`. `skipped` and `unknown` are kept apart deliberately: collapsing "not applicable" into "no problem found" is how a diagnostic ends up reporting health it never established. The three discovery rungs are where that distinction earns its keep: they answer `skipped` when auto-discovery is switched off, but `unknown` when the stored discovery config did not parse — calling an unreadable setting a deliberate switch-off states as chosen something the operator never chose. And "nothing is blocking it, your settings are just strict" is a valid verdict — when nothing is provable, the report says so rather than inventing a cause.

A rung that throws resolves to `unknown` under a fixed sentence; the throw's own message is never persisted or served, because the run row is read by an operator who cannot act on a stack-shaped string and is served by GETs that stay open under `LIVE_DEMO`. The message still has to reach someone, so the rung runner hands it to its caller and the worker logs it at error level with the step id. Without that hand-off a rung failing on every single run would be invisible in the report and in the logs alike.
