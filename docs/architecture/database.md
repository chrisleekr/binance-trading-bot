...
Stranding is also version-specific, which is why the `db-isolation` CI job pins its Postgres service to the deployed TimescaleDB version rather than tracking `latest-pg17` like the other jobs. Measured across images: through **2.27.1** a row inserted while `timescaledb.restoring` is on survives in the parent heap, and from **2.28.0** it is discarded, so the fixture cannot be built on a newer server at all. A migration suite run against `latest` therefore both validates a database nobody runs and silently loses this test. The test names the server version when it cannot strand a row, so the pin falling out of date reads as a clear failure rather than a null-constraint error from the next migration.
