...
One measured subtlety, because it decides whether a regression test is worth anything: the parent is excluded only once the hypertable owns **at least one chunk**. With zero chunks there is nothing to expand to, the parent stays in the plan, and the backfill does reach the root heap. A test that seeds nothing but a stranded row therefore passes against an unfixed migration; `packages/db/__tests__/action-logs-root-heap-migration.test.ts` seeds a normally-routed row first so a chunk exists, and asserts that chunk count is non-zero.

Stranding is also version-specific, which is why the `db-isolation` CI job pins its Postgres service to the same image digest `deploy/compose/docker-compose.prod.yml` and `packages/testcontainers` pin (TimescaleDB **2.26.4**) rather than tracking `latest-pg17` like the other jobs. Measured across images: through **2.27.1** a row inserted while `timescaledb.restoring` is on survives in the parent heap, and from **2.28.0** it is discarded, so the fixture cannot be built on a newer server at all. One digest across all three keeps the CI lane and the hermetic `TESTCONTAINERS=1` lane from disagreeing about whether this test can run. A migration suite run against `latest` therefore both validates a database nobody runs and silently loses this test. The test names the server version when it cannot strand a row, so the pin falling out of date reads as a clear failure rather than a null-constraint error from the next migration.
