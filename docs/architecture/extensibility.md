...
| `requiredWindow?` | Optional pure maximum candle lookback (in candles, on the strategy's candle interval) this config needs — the longest moving average, window-low scan, or z-score lookback the config enables. The worker sizes the `tick()` candle window from it in **both** live and backtest, so a lookback larger than the default window is honoured everywhere rather than silently starved live yet active in backtest. Reads the config **defensively** (the live worker may pass it unparsed) and never throws. The caller floors the result at the default window, so a smaller value never shrinks it, and **caps** it at `MAX_CANDLE_WINDOW` — the most closed candles one Binance klines request can supply (its `limit` maxes at 1000 and the newest row is still forming, so 999). A config demanding more does not get more; it gets the ceiling. So a strategy whose gate **fails closed** on a short window (holds rather than trades) MUST bound its own window knobs inside the ceiling in `configSchema`, or an operator can save a config that never trades while reporting warm-up. A strategy that clamps to whatever candles arrive is not blocked that way, but its knob is still bounded so the number it advertises is one the pipeline can deliver. A maximum added to one of these knobs binds at **validation** time only and does not repair a profile row already stored above it: that profile keeps ticking, because the live tick path reads its stored config unparsed, but it can no longer be re-saved and it now carries an unparseable config everywhere a path does parse — including the fail-closed disposal described under `attributeOrder` below, where it costs the **fingerprint** proof of ownership. Disposal claims an order on **either** of two independent proofs, so the other still holds: an order this profile recorded in `orders` is still claimed and cancelled, a protective stop placed through the executor included. What an unparseable config abandons is the order with **no** local row — the one a bookkeeping failure left live, for which the fingerprint was the only remaining claim. That order survives the delete, and whether it is even announced is conditional: the residual announcement is scoped to the symbols the profile still carries a live order row or a position row for, so an abandoned order on one of those — the common case, where the position row is still there — is announced rather than cancelled, while one sitting on a symbol the profile has since unbound with no local row of any kind falls in neither bucket: not cancelled, not announced, no log line. The operator's check after such a delete is the Orphan orders page, not the log. So tightening one of these maxima means checking the rows already stored, not just shipping the schema — [Troubleshooting](../operations/troubleshooting.md#a-stored-config-now-exceeds-a-schema-maximum) carries the read-only audit query for it. Absent on strategies whose lookback never exceeds the default. |
...
A strategy that emits an order **because of** an override stamps that override's id on the order: `intent.overrideActionId`. This is the ONLY thing that lets the worker tie an order's real outcome back to the override the operator is watching. Without it the worker can settle the override only on the strategy's INTENT to act, so an order that was throttled, rejected by Binance, or dropped by an entry breaker still reads as "done". Attribution is by id equality and never by a heuristic — "a BUY got dropped, it was probably the override's" settles the wrong row the moment a tick emits an unrelated order, which is every grid tick.
