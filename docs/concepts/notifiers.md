...
Each profile has its own set of alert categories. Every category defaults to **on** except `order-filled` (off — an active grid fills often), so a new profile keeps alerting on everything until you mute a category. Muting drops the notification but never the underlying action: the entry breakers still halt, discovery still rotates. A change takes effect with no restart.
...
| `loss-guard-halt` | warn | The loss-streak guard or the drawdown guard trips and pauses new buys for its configured pause. One alert per pause, not one every 30s while paused; a pause that expires and immediately re-trips is a new pause and alerts again. |
...
| `discovery-health` | warn | Auto-discovery looks wedged. Fires on staleness (the discovery cron produced no snapshot in over 2× the profile's refresh period, so scanning has stopped) or a persistent breadth block (the latest 8 cycles all had the market-breadth gate off, so every add was blocked and the floor may be unreachable for this quote). The staleness half stays silent while an [asset-policy abort](discovery.md) is parked and still recent enough to explain the missing scans, because that fault has its own finding and its own alert; the breadth half is never suppressed. Throttled to one alert per trigger per hour. Separate from `discovery`: mute the info-level rotation chatter without losing the warn-level "discovery stopped working" alert. |
