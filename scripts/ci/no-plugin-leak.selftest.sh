#!/usr/bin/env bash
# Self-test for no-plugin-leak.sh. Drives the real gate over the fixture trees under __fixtures__/plugin-leak/ via the GUARD_ROOT override, so what is proven is the shipping gate and not a copy of it.
#
# Scope is the two walk stops, which is what this gate had no way to prove before: it carried no floor at all: a union of the two app trees meant apps/worker vanishing left the raw-hit count healthy on apps/api alone, and the invariant was reported holding over an app never opened.
#
...
empty_walk_out="$(GUARD_ROOT="$dir/__fixtures__/plugin-leak/reject-empty-walk" bash "$gate" 2>&1)"
empty_walk_rc=$?
if [ "$empty_walk_rc" -eq 0 ] || ! grep -qF 'scan matched no .ts/.tsx files under apps/api, apps/worker —' <<<"$empty_walk_out"; then
...
narrowed_walk_out="$(GUARD_ROOT="$dir/__fixtures__/plugin-leak/reject-narrowed-walk" bash "$gate" 2>&1)"
narrowed_walk_rc=$?
if [ "$narrowed_walk_rc" -eq 0 ] || ! grep -qF 'walk narrowed' <<<"$narrowed_walk_out" || ! grep -qF 'apps/worker/src/strategies.ts' <<<"$narrowed_walk_out"; then
  echo "FAIL: reject-narrowed-walk expected the anchor stop naming apps/worker/src/strategies.ts (rc=$narrowed_walk_rc)"
  echo "$narrowed_walk_out"
  fails=1
fi

if [ "$fails" -ne 0 ]; then
  echo "no-plugin-leak self-test: FAILED"
  exit 1
fi
echo "no-plugin-leak self-test: OK"
