#!/usr/bin/env bash
# Self-test for no-invalid-mermaid.sh. Drives the real gate over the fixture trees under __fixtures__/mermaid/ via the GUARD_ROOT override, so what is proven is the shipping gate and not a copy of it.
#
# Scope is the two walk stops, which is what this gate had no way to prove before: a docs re-layout that leaves the diagram pages out of scope still returns plenty of markdown, so the page count stays healthy and the Mermaid rules go unchecked.
#
...
gate="$dir/no-invalid-mermaid.sh"
...
empty_walk_out="$(GUARD_ROOT="$dir/__fixtures__/mermaid/reject-empty-walk" bash "$gate" 2>&1)"
empty_walk_rc=$?
...
narrowed_walk_out="$(GUARD_ROOT="$dir/__fixtures__/mermaid/reject-narrowed-walk" bash "$gate" 2>&1)"
narrowed_walk_rc=$?
...
if [ "$fails" -ne 0 ]; then
  echo "no-invalid-mermaid self-test: FAILED"
  exit 1
fi
echo "no-invalid-mermaid self-test: OK"
