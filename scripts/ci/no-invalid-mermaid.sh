...
cd "$root"
# Overridable so no-invalid-mermaid.selftest.sh can drive this exact script over fixture trees rather than re-implementing its matching.
...
// Walked and vacuity-checked by the shared helper: it refuses a walk that returns nothing AND one that still returns pages but no longer reaches docs/index.md, the docs home page, which carries a Mermaid diagram of its own. The second stop is the one a page count cannot express — a docs re-layout leaves plenty of markdown in scope while the pages this rule was written for go unscanned.
