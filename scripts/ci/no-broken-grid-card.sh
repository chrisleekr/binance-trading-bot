...
// Walked and vacuity-checked by the shared helper: it refuses a walk that returns nothing AND one that still returns pages but no longer reaches docs/index.md, which is the only page in the tree that uses a card grid. Losing it leaves this gate scanning every other page for a construct none of them contain, and a page count cannot tell that apart from a clean tree.
