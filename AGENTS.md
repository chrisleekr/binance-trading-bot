...
- **`Decision` union stays generic.** `noop` / `place-order` / `cancel-order` / `replace-order` / `emit-event` / `set-kv` / `delete-kv` (`packages/strategy/core/src/decision.ts`). Generic exchange-request shapes such as `replace-order` belong in the union; strategy-specific side effects ride `emit-event` with namespaced keys, not new strategy-specific variants; cross-symbol coupling rides `set-kv` / `delete-kv`. A removed variant takes its worker-side port with it — no idle seams.
