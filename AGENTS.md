...
8. Migrations: hand-authored `NNNN_*.sql` applied in order by the checksum-tracked runner (`packages/db/src/migrate.ts`, tracked in `_app_migrations`); the `packages/db` suite runs `migrate()` against real Postgres. A shipped migration is **immutable** — renaming or editing one (a comment counts) wedges every already-migrated database at that file, and no later migration can repair it. Change it with a new migration; `no-mutated-applied-migration` enforces this because a fresh-DB suite cannot.
