...
Prerequisites: [Bun](https://bun.sh) 1.4+ (pinned to `1.4.2` in `.tool-versions`; `bun.lock` is a `lockfileVersion: 2` file that Bun 1.3.x cannot parse) and Docker (with the `compose` plugin) for the local Postgres + Redis stack.
