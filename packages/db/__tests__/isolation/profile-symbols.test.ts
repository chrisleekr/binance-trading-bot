...
/**
 * Discovery source plumbing for Slice 1: an attached symbol defaults to
 * `manual`, `setSource` (the Pin action) flips it, and `removeAutoIfFlat`
 * enforces the safe-removal contract — discovery may reap a symbol only when it
 * is `auto` AND flat (zero held quantity, no open orders). The flatness inputs
 * (avg_entry_prices, orders) are seeded directly so each branch is exercised.
 */
describeIfDb('profile-symbols discovery source + flat-guard', () => {
  let fx: IsolationFixture;
  let ap: ProfileRepo;
  ...
  it('a freshly attached symbol defaults to source=manual', async () => {
  ...
  it('setSource flips source and is idempotent; returns null for a missing symbol', async () => {
  ...
  it('removeAutoIfFlat returns not-found for an unattached symbol', async () => {
    expect(await ap.profileSymbols.removeAutoIfFlat('GHOSTUSDT')).toBe('not-found');
  });

  it('removeAutoIfFlat refuses to reap a manual symbol', async () => {
    await ap.profileSymbols.upsert('CCCUSDT', 'CCC', { overrideConfig: null }); // defaults manual
    expect(await ap.profileSymbols.removeAutoIfFlat('CCCUSDT')).toBe('not-auto');
    expect(await ap.profileSymbols.findForSymbol('CCCUSDT')).not.toBeNull();
  });

  it('removeAutoIfFlat reaps an auto symbol that is flat', async () => {
    await ap.profileSymbols.upsert('DDDUSDT', 'DDD', { overrideConfig: null });
    await ap.profileSymbols.setSource('DDDUSDT', 'auto');
    expect(await ap.profileSymbols.removeAutoIfFlat('DDDUSDT')).toBe('removed');
    expect(await ap.profileSymbols.findForSymbol('DDDUSDT')).toBeNull();
  });

  it('removeAutoIfFlat refuses an auto symbol holding a position (quantity > 0)', async () => {
    ...
    expect(await ap.profileSymbols.removeAutoIfFlat('EEEUSDT')).toBe('held');
    expect(await ap.profileSymbols.findForSymbol('EEEUSDT')).not.toBeNull();
  });

  it('removeAutoIfFlat refuses an auto symbol with an open order', async () => {
    ...
    expect(await ap.profileSymbols.removeAutoIfFlat('FFFUSDT')).toBe('held');
    expect(await ap.profileSymbols.findForSymbol('FFFUSDT')).not.toBeNull();
  });

  it('removeAutoIfFlat refuses an auto symbol holding a position AND an open order', async () => {
    ...
    expect(await ap.profileSymbols.removeAutoIfFlat('IIIUSDT')).toBe('held');
    expect(await ap.profileSymbols.findForSymbol('IIIUSDT')).not.toBeNull();
  });

  it('removeAutoIfFlat treats a zero-quantity ledger row as flat', async () => {
    ...
    expect(await ap.profileSymbols.removeAutoIfFlat('GGGUSDT')).toBe('removed');
  });

  it('removeAutoIfFlat ignores a closed order (closed_at set) when judging flatness', async () => {
    ...
    expect(await ap.profileSymbols.removeAutoIfFlat('HHHUSDT')).toBe('removed');
  });
});
