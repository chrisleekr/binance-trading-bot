 ...
 *
 * One value serves all three kinds, but NOT because only `api` queues. The worker's queue concurrencies sum well past its 25 connections (`tick` alone is 25, before the pipeline, the advisor and ~30 cron queues), so a worker checkout can queue too, and now rejects instead of waiting forever. The difference is what happens next: the api translates the rejection into a 503 (`apps/api/src/middleware/error.ts`), while on the worker it reaches BullMQ as an ordinary job failure, so a queue with `attempts: 1` drops that pass rather than retrying backpressure. `admin` is `max: 2` with no concurrent fan-out, and migrations do not touch a pool at all (`migrate()` opens a bare `Client`). Per-kind numbers would be three knobs with no production load to tune two of them against; the case for splitting them is worker retry behaviour, not the deadline itself.
 *
