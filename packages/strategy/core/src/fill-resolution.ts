 ...
 *
 * A bare `isBelowMinNotional` is the wrong choice, because declining to CREATE a position and destroying an existing cost basis are not the same decision and do not deserve the same bar. A balance worth most of one minimum order is what a `rebalance` target weight looks like, and it is also what an operator's base-asset RESERVE leaves behind once the reserve is drained out of the bot's view — real coins, a real cost basis, and a number the operator can undo by lowering the reserve. Residue is orders of magnitude smaller: the live ENAUSDT crumb was 0.027% of its symbol's floor while a reserved holding sits at tens of percent of it. So the floor itself supplies the denominator the share test lost, at the same 1% ratio the share test uses.
 *
